...
classicMower::classicMower()
{
    flag = false;
    speed = 270.0 * 33 / 1000;
}

QRectF classicMower::boundingRect() const
{
    return QRectF(-30, -30, 60, 60);
}

void classicMower::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    Q_UNUSED(option)
    Q_UNUSED(widget)
...
void classicMower::advance(int phase)
{
    if (!phase)
        return;
    update();
